// this is my src\components\mainweathercard.jsx file
...
const MainWeatherCard = ({ weatherData }) => {
  ...
  const currentDate = timestamp
    ...
    return (
      <div style={{ backgroundColor: '#4B5563', color: 'white', borderRadius: '0.5rem', width: '160px', padding: '30px', paddingBottom: '20px' }}>
        <div style={{fontSize:'20px', fontWeight: 'bold'}}>Now</div>
        <div style={{ display: 'flex', alignItems: 'center', fontSize: '35px', fontWeight: 'bold', paddingBottom: '10px' }}>
          {temperatureCelsius}°C {renderTemperatureIcon()}
        </div>
        <div style={{ fontSize: '15px', marginTop: '8px', fontWeight: '500' }}>{weatherDescription}</div>
